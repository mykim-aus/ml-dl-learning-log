...
## 140. 3-클래스 출력엔 sigmoid 대신 softmax — 3개를 "합=1" 확률로

- 문제: 출력이 3개(클래스별 점수)인데 "셋 중 하나"라 합=1 필요. sigmoid를 3개에 각각 적용하면 각자 0~1이나 **합이 1 보장 안 됨**(각 sigmoid가 자기 값만 보고 독립 계산). 예 z=[3,3,3]→sigmoid[.95,.95,.95] 합2.86 ❌.
- **softmax** = sigmoid의 여러-클래스 버전. ① 각 점수를 e^(점수)로(양수화+차이 증폭) → ② 전체 합으로 나눔 → 자동 합=1. 공식 softmax(z)_i = e^(z_i)/Σ_j e^(z_j). (합=1 되는 건 ② "전체 합으로 나누기" 덕분)
- 예: softmax([3,3,3])=[.33,.33,.33] 합1, softmax([3,1,0])=[.84,.11,.04] 합1(큰 점수에 큰 확률).
- 순전파 마지막: output=softmax(z2). sigmoid는 softmax의 2-클래스 특수case(형제). 노트 122 "출력층 활성화"가 다중클래스로 확장된 것.
- (KIM 도출·정정) 핵심="3개 전체를 함께 봄"이 맞음(그래서 합=1, sigmoid와 다름). 단 "평균" 아니라 "합으로 나누기": 각자÷합=비율, 다시 더하면 합/합=1. 예[8,1,1]→[.8,.1,.1]. (구현: softmax는 행별 axis=1, max 빼기는 지수폭발 방지)

## 141. 다중클래스 손실 = cross-entropy (= -log(정답클래스 확률)) + one-hot 정답

- 샘플 손실 = **-log(정답 클래스에 준 확률)**. 정답에 1.0 주면 -log1=0(벌점0), 쥐꼬리(0.012) 주면 -log0.012≈4.4(큼). 전체 손실 = 그 평균. (KIM: 오차 심하면 벌점 큼)
- = **cross-entropy(교차 엔트로피)** = BCE의 다중클래스 버전(BCE는 2클래스 특수case). 같은 -log 아이디어.
- **one-hot**: y 정수(0/1/2)를 벡터로 — 0→[1,0,0],1→[0,1,0],2→[0,0,1] (노트 135의 "정답 [1,0,0]" 형식). `np.eye(3)[y]`.
- 계산: loss = -mean(sum(one-hot × log(output), axis=1)). one-hot이 정답 클래스만 남기고(나머지 ×0) 그 클래스 log확률만 골라냄 → 결국 -log(정답확률) 평균.
- 학습 전(랜덤 가중치)엔 손실 큼(노트 123). [나중에: softmax+cross-entropy의 dz2 = (output − one-hot)/N — XOR의 (output−y)/N과 동일 형태로 깔끔]

## 142. "벌점" = 손실(loss). cross-entropy·BCE·MSE는 손실을 재는 방법(종류)

- (KIM 질문) cross-entropy 벌점 = 여기서 손실? → **응, 벌점 = 손실(loss) = 같은 것.**
- **손실(loss)**: 예측이 정답과 얼마나 다른지 재는 숫자 1개. 낮을수록 좋음. **학습 = 이 손실을 낮추기.**
- **cross-entropy/BCE/MSE = 손실을 계산하는 방법(공식)들.** 문제 종류로 골라 씀: 회귀(연속값)→**MSE**(제곱오차, Day 2) / 2-클래스→**BCE**(XOR) / 다중클래스→**cross-entropy**(Iris).
- cross-entropy = "손실 재는 여러 자 중 지금 쓰는 자".

## 143. train/test 분리 — 자르기 전 셔플 필수(정렬 gotcha) + 정규화는 train 통계로

- 왜 분리: 안 본 데이터로 일반화 측정(노트 128·130). 150 → 120 train / 30 test.
- **gotcha**: Iris는 클래스 순 정렬(앞50 setosa·중50 versicolor·뒤50 virginica). 안 섞고 "앞120/뒤30" 자르면 **test가 전부 virginica**([0,0,30]) → 다른 종 평가 불가·train도 virginica 부족. (KIM: 랜덤하게 섞어서)
- **해법**: 자르기 전 `permutation`으로 셔플 → X·y 같은 순서로. 결과 test 분포 [10,11,9] 골고루.
- **정규화 fix(노트 139 약속)**: 평균·표준편차는 **X_train으로만** 계산해 train·test 둘 다 적용. test 통계 미리 보면 반칙(data leakage) — test는 "안 본 데이터"여야(노트 130).

## 144. Iris NN 완성 — 학습으로 7%→train 99%/test 97%, 일반화가 눈에 보임

- 결과: 학습 전 train 0.075·test 0.067(랜덤) → 학습 후 **train 0.992·test 0.967**, loss 2.80→0.03. (KIM 예측대로 "가중치 조절"=backprop이 해냄)
- **일반화 목격(노트 128·130)**: test 96.7%는 **학습에 한 번도 안 쓴 30송이**에서 나온 점수 → 외운 게 아니라 규칙을 배움.
- **과적합 아님(노트 130·131)**: train 99.2% vs test 96.7% = 격차 작음. 외웠다면 train~100%·test 뚝. 작은 격차=건강한 일반화.
- loss 2.80→0.036 급락 후 평평 = 수렴(노트 129 A, 다 맞혀서 멈춤).
- 구조: 입력4→은닉8→출력3, softmax+cross-entropy, backprop은 XOR과 동일 형태(shape만 다름). argmax로 예측 클래스 뽑아 정확도 측정.
- 코드: `code/iris-numpy/iris.py`.
